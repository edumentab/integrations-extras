...
def create_database(tls=False):
    ...
    status_command = 'docker exec fdb-0 fdbcli --exec "status json"' if not tls else 'docker exec fdb-tls fdbcli -C /var/fdb/fdb.cluster --tls_certificate_file /var/fdb/fdb.pem --tls_key_file /var/fdb/private.key --tls_verify_peers Check.Valid=0 --exec "status json"'
    ...
    if not status.get('client').get('database_status').get('available'):
        command = 'docker exec fdb-0 fdbcli --exec "configure new single memory"' if not tls else 'docker exec fdb-tls fdbcli -C /var/fdb/fdb.cluster --tls_certificate_file /var/fdb/fdb.pem --tls_key_file /var/fdb/private.key --tls_verify_peers Check.Valid=0 --exec "configure new single memory"'
